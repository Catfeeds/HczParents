...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware) {
		displayImage(uri, imageAware, null, null, null, -1, -1, -1);
	}
	...
	public void displayImage(String uri, ImageAware imageAware,
		...
		displayImage(uri, imageAware, null, listener, null, -1, -1, -1);
	}
	...
	public void displayImage(String uri, ImageAware imageAware,
		...
		displayImage(uri, imageAware, options, null, null, -1, -1, -1);
	}
	...
	public void displayImage(String uri, ImageAware imageAware,
		...
		displayImage(uri, imageAware, options, listener, null, -1, -1, -1);
	}
	...
	public void displayImage(String uri, ImageAware imageAware,
			DisplayImageOptions options, ImageLoadingListener listener,
			ImageLoadingProgressListener progressListener, int width,
			int height, int isBig) {
		checkConfiguration();
		if (imageAware == null) {
		...
		if (TextUtils.isEmpty(uri)) {
			...
			if (options.shouldShowImageForEmptyUri()) {
				...
				imageAware.setImageDrawable(null, uri);
			}
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(
					engine, imageLoadingInfo, defineHandler(options), width,
					height, isBig);
			if (options.isSyncLoading()) {
				displayTask.run();
			} else {
	...
	@SuppressWarnings("deprecation")
	public Bitmap getBitmapFromMemory(String url, ImageView imageAware) {
		// ImageSize targetSize = ImageSizeUtils
		// .defineTargetSizeForView(new ImageViewAware(imageAware),
		// configuration.getMaxImageSize());
		// String memoryCacheKey = MemoryCacheUtils.generateKey(url,
		// targetSize);
		// Bitmap bm = configuration.memoryCache.get(url);
		// if (bm != null)
		// imageAware.setImageBitmap(bm);
		return configuration.memoryCache.get(url);
	}
	...
	public void displayImage(String uri, ImageView imageView) {
		displayImage(uri, new ImageViewAware(imageView), null, null, null, -1,
				-1, -1);
	}
	...
	public void displayImage(String uri, ImageView imageView,
			DisplayImageOptions options, ImageLoadingListener listener,
			int width, int height, int isBig) {
		displayImage(uri, imageView, options, listener, null, width, height,
				isBig);
	}
	...
	public void displayImage(String uri, ImageView imageView,
			...
			int height, int isBig) {
		displayImage(uri, new ImageViewAware(imageView), options, listener,
				progressListener, width, height, isBig);
	}
	...
	public void loadImage(String uri, ImageSize targetImageSize,
		...
		NonViewAware imageAware = new NonViewAware(uri, targetImageSize,
				ViewScaleType.CROP);
		displayImage(uri, imageAware, options, listener, progressListener, -1,
				-1, -1);
	}
