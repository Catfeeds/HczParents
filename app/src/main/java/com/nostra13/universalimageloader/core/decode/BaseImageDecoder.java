...
public class BaseImageDecoder implements ImageDecoder {
	...
	public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
		...
		try {
			...
			if(null!=decodingInfo.getOriginalImageUri()&&decodingInfo.getOriginalImageUri().startsWith("file://")&&decodingInfo.getIsBig()>0){
				String path=decodingInfo.getOriginalImageUri().replace("file://", "");
				decodingOptions.inJustDecodeBounds = true;
				BitmapFactory.decodeFile(path, decodingOptions);
				int width=decodingInfo.getWidth();
				int height=decodingInfo.getHeight();
//				int isBig=decodingInfo.getIsBig();
				decodingOptions.inSampleSize = calculateInSampleSize(decodingOptions, width<0?Constants.UNIVALSAL_DEFAULT_WIDHT:width, height<0?Constants.UNIVALSAL_DEFAULT_HEIGHT:height);
				decodingOptions.inJustDecodeBounds = false;
				decodedBitmap = BitmapFactory.decodeFile(path, decodingOptions);
			}else
	...
	protected Options prepareDecodingOptions(ImageSize imageSize,
		...
		if (scaleType == ImageScaleType.NONE) {
			// 大长图不压缩
			if (imageSize.getWidth() / imageSize.getHeight() > 4
					|| imageSize.getHeight() / imageSize.getWidth() > 4) {
				scale = 1;
			} else {
