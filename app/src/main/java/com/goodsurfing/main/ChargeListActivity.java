...
public class ChargeListActivity extends BaseActivity {
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					if (com.goodsurfing.constants.Constants.isNetWork)
						getPayTime();
					else
						Toast.makeText(ChargeListActivity.this, "您的网络已断开，请检查网络", Toast.LENGTH_SHORT).show();
					// TODO 支付成功
					Toast.makeText(ChargeListActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
				} else {
					// 判断resultStatus 为非"9000"则代表可能支付失败
					// "8000"代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
					if (TextUtils.equals(resultStatus, "8000")) {
						Toast.makeText(ChargeListActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						Toast.makeText(ChargeListActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
					}
				}
				break;
			}
			case SDK_CHECK_FLAG: {
				Toast.makeText(ChargeListActivity.this, "检查结果为：" + msg.obj, Toast.LENGTH_SHORT).show();
				break;
			}
			default:
				break;
		...
		new PayOkServer(new DataServiceResponder() {
			...
			public void onResult(DataServiceResult result) {
				if ("0".equals(result.code)) {
					Intent intent = new Intent(ChargeListActivity.this, PayOkActivity.class);
					...
					ActivityUtil.showPopWindow4Tips(ChargeListActivity.this, title_layout, false, result.extra+"");
				}
			}
	...
	public void pay(View v) {
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(ChargeListActivity.this);
				// 调用支付接口，获取支付结果
	...
	public void check(View v) {
		Runnable checkRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask payTask = new PayTask(ChargeListActivity.this);
				// 调用查询接口，获取查询结果
	...
	private class GetPrepayIdTask extends AsyncTask<Void, Void, Map<String, String>> {
		...
		protected void onPreExecute() {
			dialog = ProgressDialog.show(ChargeListActivity.this, "提示", "正在获取预支付订单...");
		}
