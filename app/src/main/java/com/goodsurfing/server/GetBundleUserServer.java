 ...
 * 
 */
public class GetBundleUserServer extends BaseDataService {
	Context context;

	public GetBundleUserServer(DataServiceResponder responder, String url,
			Context context) {
		...
		try {
			...
			if ("0".equals(errCode)) {
				JSONArray listJson = jsonObject
						.getJSONArray("binduser");
				Constants.user.clear();// "id":"1",
