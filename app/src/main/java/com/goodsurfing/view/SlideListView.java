 ...
 * 侧滑 listview
 * 
 * Copyright: 版权所有 (c) 2014 Company: 北京易微网络
 * 
 * @author xzj
...
public class SlideListView extends ListView {
	...
	/** 左侧菜单的长度 */
	private int leftLength = 120;
	/** 右侧菜单的长度 */
	private int rightLength = 120;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (Constants.canMoveAgin) {
				Constants.canMoveAgin = false;
			}
			downX = (int) ev.getX();
			downY = (int) ev.getY();
			...
			if (this.mode == MOD_BOTH) {
				...
				if (this.leftLength == 0) {
					this.leftLength = context.getResources()
							.getDimensionPixelSize(R.dimen.item_right_delete);
				}
				if (this.rightLength == 0) {
					this.rightLength = context.getResources()
							.getDimensionPixelSize(R.dimen.item_right_delete);
				}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (canMove) {
				...
				if (deltaX < 0
						&& (this.mode == MOD_BOTH || this.mode == MOD_LEFT)) {
					/* 向左滑 */
					if (deltaX < -leftLength)
						itemView.scrollTo(-leftLength, 0);
					else
	...
	private void scrollBack() {
		isSlided = false;
		if (null != itemView && scroller != null) {
