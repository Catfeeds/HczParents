...
public class LocationDialogBuilder extends NiftyDialogBuilder implements
	...
	private static LocationDialogBuilder instance;
	...
	public LocationDialogBuilder(Context context, int theme) {
		super(context, theme);
	...
	public static LocationDialogBuilder getInstance(Context context) {
		...
		if (instance == null || ((Activity) context).isFinishing()) {
			synchronized (LocationDialogBuilder.class) {
				if (instance == null) {
					instance = new LocationDialogBuilder(context,
	...
	private void initDialog() {
		rlCustomLayout = (RelativeLayout) LayoutInflater.from(context).inflate(
				R.layout.layout_location_dialog, null);
		areasWheel = (AreasWheel) rlCustomLayout
				.findViewById(R.id.aw_location_wheel);
		setDialogProperties();
	}
